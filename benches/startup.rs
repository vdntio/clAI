...
/// Helper to reset config cache when bench feature is enabled
#[cfg(feature = "bench")]
fn reset_cache() {
...
fn reset_cache() {
    // No-op when bench feature not enabled
}
...
fn benchmark_history_reading(c: &mut Criterion) {
    ...
    group.bench_function("read_history_tail_1000_lines", |b| {
        b.iter(|| {
            let _history =
                clai::context::history::read_history_tail(black_box(&history_path), black_box(100));
        });
    });
