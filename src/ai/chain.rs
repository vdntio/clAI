...
impl std::fmt::Debug for ProviderChain {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("ProviderChain")
            ...
            .field("provider_instances", &format!("<{} cached>", self.provider_instances.lock().unwrap().len()))
    ...
    async fn complete(&self, request: ChatRequest) -> Result<ChatResponse> {
        ...
        for (index, provider_name) in self.providers.iter().enumerate() {
            ...
            if !provider.is_available() {
                last_error = Some(anyhow::anyhow!("Provider {} is not available", provider_name));
                continue;
            }
        ...
        Err(last_error.unwrap_or_else(|| {
            anyhow::anyhow!("All providers in chain failed")
        }))
    }
