    ...
    match gather_context(&config) {
        Ok(json_str) => {
            ...
            let parsed: serde_json::Value =
                serde_json::from_str(&json_str).expect("Context should be valid JSON");
            ...
            assert!(
                parsed.get("stdin").is_some(),
                "Stdin field should be present"
            );
            ...
            assert!(
                system.contains_key("architecture"),
                "System should have architecture"
            );

            // Verify cwd is a string
            assert!(
                parsed.get("cwd").unwrap().is_string(),
                "CWD should be a string"
            );

            // Verify files is an array
            assert!(
                parsed.get("files").unwrap().is_array(),
                "Files should be an array"
            );

            // Verify history is an array
            assert!(
                parsed.get("history").unwrap().is_array(),
